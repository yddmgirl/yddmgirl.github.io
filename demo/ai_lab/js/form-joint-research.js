$(function(){
    ...
    var langTip = ['Please enter ','Please enter correct email','Please select ','Please choose ','file format error','Reupload','Uploading','Please upload ','Delete','Success','Error','File is large'];
    ...
    $(document).on('change','.inp-upload',function(e){
        try{
            ...
            alert('抱歉，您的浏览器不支持上传，推荐使用chrome！\nYour browser is unsupported. We recommend using Chrome. ');
            return;
        }
        if(!fileName) return;
        var $this = $(this);
    ...
    function checkInpText(val,_thisInp){
        ...
        if(val == ''){
            tipShow(_thisInp, "You can't leave this empty.");
    ...
    function checkSelect(_thisA){
        ...
        if(!_thisA.hasClass('selected')){
            tipShow(_thisA, "You can't leave this empty.");
        }
    }
    ...
    function checkFile(_this){
        ...
        if(_thisUploadInp.prop('currentFileAmount') < _thisUploadInp.prop('minAmount')){
            tipShow(_thisUploadInp, "You can't leave this empty.");
        }
    }
